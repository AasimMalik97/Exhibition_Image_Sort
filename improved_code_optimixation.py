...
class LargeScaleProcessor:
    ...
    def sort_images_fast(self) -> List[ImageData]:
        """
        Optimized image sorting without using multiprocessing.
        """
        ...
        while len(sorted_images) < len(final_list):
            ...
            if not candidate_indices:
                # If no candidates, add first unused image
            ...
            for idx in candidate_indices:
                candidate_tags = final_list[idx].tags
                similarity = self.calculate_similarity(current_tags, candidate_tags)
    ...
    def calculate_similarity(self, current_tags: Set[str], candidate_tags: Set[str]) -> float:
        """
        Calculate the similarity between two sets of tags.
        This function can be adjusted to improve scoring.
        """
        # Simple intersection for similarity
        common_tags = current_tags & candidate_tags
